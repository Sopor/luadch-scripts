--[[
    ...
    usage:
        [+!#]setemail <emailaddress>            <--- add your own email address
        [+!#]setemail ignore                    <--- ignore the reminder of adding an email address
        [+!#]setemail verify                    <--- verify your own email address
        [+!#]setemail delete                    <--- delete your own email address

 op usage:
        [+!#]setemail add <nick> <emailaddress> <--- add an email address for any user (also as RC)
        [+!#]setemail showop <nick>             <--- show an email address for any user (also as RC)
        [+!#]setemail deleteop <nick>           <--- delete an email address for any user (also as RC)
        [+!#]setemail verifyop <nick>           <--- verify an email address for any user (also as RC)
        [+!#]setemail ignoreop <nick>           <--- ignore the reminder for any user (also as RC)
        [+!#]setemail whois <emailaddress>      <--- show a user's email address
        [+!#]setemail showall                   <--- show all email addresses
        [+!#]setemail checkusers                <--- check if email addresses have an account
        [+!#]setemail showdeleted               <--- show email addresses without an account (run after checkusers)
        [+!#]setemail purgedeleted              <--- delete email addresses without an account (run after checkusers)
...
local msg_denied = "[ EMAIL ] --> You are not allowed to use this command."
local msgsv_denied = "[ EMAIL ] --> Du har inte tillåtelse att använda det här kommandot."
...
local msg_delcancel = "[ EMAIL ] --> Deletion cancelled. To delete your email, type: [+!#]setemail delete DELETE"
local msgsv_delcancel = "[ EMAIL ] --> Borttagning avbruten. För att ta bort din e-postadress, skriv: [+!#]setemail delete DELETE"
...
local msg_op_checked = "[ EMAIL ] --> User check completed. Active users: "

local msg_op_deleted = ", deleted users: "

local msg_op_nodeleted = "[ EMAIL ] --> No deleted users found!"

local msg_op_purgeusage = "[ EMAIL ] --> Type: [+!#]setemail purgedeleted DELETE"

local msg_op_purged = "[ EMAIL ] --> Removed deleted users: "

local help_op_desc = "Lets users save their email address."

local help_op_showop = "Usage: [+!#]setemail showop <nick>"

local help_op_deleteop = "Usage: [+!#]setemail deleteop <nick>"

local help_op_verifyop = "Usage: [+!#]setemail verifyop <nick>"

local help_op_ignoreop = "Usage: [+!#]setemail ignoreop <nick>"

local help_op_whois = "Usage: [+!#]setemail whois <emailaddress>"
...
local help_desc = "Lets users save their email address."
...
=== SET EMAIL ==================================== OP USAGE ===
 ...
 [+!#]setemail showall
 [+!#]setemail checkusers
 [+!#]setemail showdeleted
 [+!#]setemail purgedeleted
...
local msg_verifyusage = "Use '[+!#]setemail verify' or the right-click menu if this is still your current email address."
local msgsv_verifyusage = "Använd '[+!#]setemail verify' eller högerklicksmenyn om detta fortfarande är din aktuella e-postadress."
...
local function savetbl()
    ...
    for cid, v in pairs(emails) do
        ...
        f:write(string.format('        ["deleted"] = %s,\n', tostring(v.deleted)))
        f:write("    },\n")
    end
    f:write("}\n")
    f:close()
...
local onbmsg = function(user, cmd, parameters, txt)
    ...
    if arg1 == "delete" then
        ...
        if utf.lower(arg2 or "") ~= "delete" then
    ...
    if level >= oplevel then
        ...
        -- checkusers
        if arg1 == "checkusers" then

            local _, regs_by_nick = hub.getregusers()

            local regs = {}

            for nick in pairs(regs_by_nick) do
                regs[utf.lower(nick)] = true
            end
            ...
            for _, v in pairs(emails) do
                ...
                if v.saved_nick and regs[utf.lower(v.saved_nick)] then
                    v.deleted = false
                    active = active + 1
                else
                    v.deleted = true
        ...
        if arg1 == "showall" then
            ...
            for _, v in pairs(emails) do
                if v.email ~= "" and v.email ~= "optout" then
                    table.insert(list, { nick = v.saved_nick, email = v.email, deleted = v.deleted, })
                end
            end
            ...
            for _, v in ipairs(list) do
                ...
                user:reply( v.nick .. " : " .. v.email .. suffix, hub_bot )
            end

            return PROCESSED
        end

        if arg1 == "showdeleted" then
            ...
            if #list == 0 then
                user:reply(msg_op_nodeleted, hub_bot)
                return PROCESSED
            end

            for _, v in ipairs(list) do
                user:reply( v.nick .. " : " .. v.email .. " (DELETED)", hub_bot )
            end

            return PROCESSED
        end

        if arg1 == "purgedeleted" then

            if utf.upper(arg2 or "") ~= "DELETE" then
                user:reply( msg_op_purgeusage, hub_bot )
                return PROCESSED
            end
            ...
            user:reply( msg_op_purged .. removed, hub_bot )
        ...
        if arg1 == "showop" then
            ...
            if arg2 == "" then
                user:reply( help_op_showop, hub_bot )
                return PROCESSED
            end
        ...
        -- deleteop
        if arg1 == "deleteop" then
        
            if arg2 == "" then
                user:reply( help_op_deleteop, hub_bot )
                return PROCESSED
            end
            for cid2, v in pairs(emails) do
                if utf.lower(v.saved_nick) == utf.lower(arg2) then
        ...
        -- verifyop <nick>
        if arg1 == "verifyop" then
        
            if arg2 == "" then
                user:reply( help_op_verifyop, hub_bot )
                return PROCESSED
            end
            for _, v in pairs(emails) do
                if utf.lower(v.saved_nick) == utf.lower(arg2) then
        ...
        -- ignoreop <nick>
        if arg1 == "ignoreop" then
        
            if arg2 == "" then
                user:reply( help_op_ignoreop, hub_bot )
                return PROCESSED
            end
            for _, v in pairs(emails) do
                if utf.lower(v.saved_nick) == utf.lower(arg2) then
        ...
        if arg1 == "whois" then
            ...
            if arg2 == "" then
                user:reply( help_op_whois, hub_bot )
                return PROCESSED
            end
            for _, v in pairs(emails) do
...
hub.setlistener("onStart", {}, function()
    ...
    ucmd.add( { "Email", "Delete email" }, cmd, { "delete", "%[line:Type 'DELETE' if you want to delete your email address]" }, { "CT1" }, minlevel )
    ...
    ucmd.add( { "Email", "OP", "Check registered users" }, cmd, { "checkusers" }, { "CT1" }, oplevel )

    ucmd.add( { "Email", "OP", "Show deleted users" }, cmd, { "showdeleted" }, { "CT1" }, oplevel )

    ucmd.add( { "Email", "OP", "Purge deleted users" }, cmd, { "purgedeleted", "%[line:Type DELETE to permanently remove deleted users]" }, { "CT1" }, oplevel )
