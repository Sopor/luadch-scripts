--[[
    ...
    etc_trafficmanager.lua by pulsar
        ...
        v2.2.3: this should be used with the included usr_upload_speed.lua
            - added 'is_blocked' to check if a user is blocked
            - fixed three bugs that make the export function not working

        v2.2.2 (drop-in patch for the 3.1.14 line):
            - also block the NAT-traversal fallback (DNAT / DRNT, via the
              ...
              connection was impossible - passive / CGNAT peers fall back
              to NAT traversal - because that path was never hooked. The
              two new listeners mirror the CTM / RCM block logic exactly
              (masterlevel gate + need_block on user and target;
              need_block tolerates a nil target). Closes the passive /
              CGNAT bypass for both transfers and CCPM. Canonical on the
              3.2.x line as etc_trafficmanager v2.8.

        v2.2.1 (drop-in patch for the 3.1.14 line, luadch-ng/luadch#502):
            - `show blocks` now also lists currently-online users who
              are auto-blocked by share (0 B / below minshare) or by a
              blocked level. These are runtime need_block()
              classifications, never persisted to block_tbl, so the old
              `show blocks` (which only iterated block_tbl) showed
              nothing for them - the operator saw an empty list plus an
              empty scripts/data/etc_trafficmanager.tbl while the users
              were in fact blocked (reported by Sopor). Online-only
              (share is a per-session INF field) and skips nicks already
              in the manual block_tbl so each user shows once.
            - `show settings` now also reports the minshare-check toggle.
            - drop-in only: bumps 2.2 -> 2.2.1. The canonical fix ships
              on master/dev as v2.7. Requires the four new lang keys
              msg_ab_* (see the accompanying etc_trafficmanager.lang.en
              additions); without them the English defaults below apply.
...
--> internal or external block
    if ( not scriptname ) or ( scriptname ~= 1 ) then -- 2.2.3
        otherScript = true --> external block
        scriptname = scriptname ~= nil and tostring( scriptname ) or msg_unknown
    end
    --> set reason msg
    reason = reason ~= nil and tostring( reason ) or msg_unknown -- 2.2.3
    ...
    --// external block
        ...
        --> add target to block tbl
        block_tbl[ firstnick ] = {}
        block_tbl[ firstnick ][ 1 ] = scriptname -- 2.2.3
        block_tbl[ firstnick ][ 2 ] = reason
        block_tbl[ firstnick ][ 3 ] = util.date()
        util.savetable( block_tbl, "block_tbl", block_file )
        --> send report
...
del = function( firstnick, scriptname, user )
    ...
    if ( not scriptname ) or ( scriptname ~= 1 ) then
        otherScript = true
        scriptname = scriptname ~= nil and tostring( scriptname ) or msg_unknown -- 2.2.3
    end
...
-- still establish a file-transfer OR CCPM connection. Mirrors CTM / RCM;
-- need_block tolerates a nil target.
