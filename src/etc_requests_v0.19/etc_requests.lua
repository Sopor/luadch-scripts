...
--> Not allowed release groups (case insensitive)
local banned_groups = {
    ["anygroup"] = true,
    ["badgroup"] = true,
    ["testgroup"] = true,
...
msg_etc_012 = lang.msg_etc_012 or "Use +filled add <release name> to fill this request within %i days.", -- Sopor
msg_etc_013 = lang.msg_etc_013 or "[ REQUESTS ]--> This release group is not allowed!", -- Sopor
...
local onbmsg = function( user, cmd, parameters, txt )
    ...
    local s1 = cmd
    local s2 = utf.match( parameters or "", "^(%a+)" )
    local s3 = utf.match( parameters or "", "^%a+ (.+%-.+)" )
          ...
          if (s3 == nil) then
            ...
            if not scene then
              ...
              elseif check_tRelFlag_D then
                ...
                -- Check banned groups
                local group = string.match(s3, "%-([^%-]+)$")
                if group then
                  group = string.lower(group)
                  if banned_groups[group] then
                    user:reply( msgs.msg_etc_013, hub_getbot)
                    return PROCESSED
                  end
                end
                local n = #requests_tbl
                local i = n + 1
                requests_tbl[ i ] = {}
...
hub.setlistener( "onStart", {},
  function()
    ...
    assert( hubcmd )
    assert( hubcmd.add( cmd_request, onbmsg ) )
    assert( hubcmd.add( cmd_filled, onbmsg ) )
    ...
    if ucmd then
      ucmd.add( { "Requests", "━━━━━━━━━━━━━━━━━━━━" }, " ", { " ", " " }, { "CT1" }, minlevel )
      ucmd.add( { "Requests", " * When you request something *" }, " ", { " ", " " }, { "CT1" }, minlevel )
      ucmd.add( { "Requests", " * use a proper release name *" }, " ", { " ", " " }, { "CT1" }, minlevel )
      ucmd.add( { "Requests", " * from a pre-site. *" }, " ", { " ", " " }, { "CT1" }, minlevel )
      ucmd.add( { "Requests", "━━━━━━━━━━━━━━━━━━━━━" }, " ", { " ", " " }, { "CT1" }, minlevel )
      ucmd.add( { "Requests", " * If the request is not a proper *" }, " ", { " ", " " }, { "CT1" }, minlevel )
      ucmd.add( { "Requests", " * release it will be DELETED! *" }, " ", { " ", " " }, { "CT1" }, minlevel )
      --ucmd.add( { "Requests", " * http://www.xrel.to *" }, " ", { " ", " " }, { "CT1" }, minlevel )
      --ucmd.add( { "Requests", " * http://predb.me *" }, " ", { " ", " " }, { "CT1" }, minlevel )
      --ucmd.add( { "Requests", " * http://predb.org *" }, " ", { " ", " " }, { "CT1" }, minlevel )
      --ucmd.add( { "Requests", " * https://www.swenews.info *" }, " ", { " ", " " }, { "CT1" }, minlevel )
      --ucmd.add( { "Requests", " * https://layer13.net *" }, " ", { " ", " " }, { "CT1" }, minlevel )
      --ucmd.add( { "Requests", " * https://pre.corrupt-net.org *" }, " ", { " ", " " }, { "CT1" }, minlevel )
      ucmd.add( { "Requests", "━━━━━━━━━━━━━━━━━━━━━━" }, " ", { " ", " " }, { "CT1" }, minlevel )
