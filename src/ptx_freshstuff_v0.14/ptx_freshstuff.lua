--[[
  ...
  ptx_freshstuff.lua converted by blastbeat
    ...
    v0.14: by Sopor
      - added 'releases new' to help and usage text
    ...
    v0.12: by Sopor
      - changed to a better explanation in the right click menu for opt-out/in
...
local send_to_pm = true  --> send bot messages to pm if use_own_bot is true
local own_botname = "#[BOT]Releases"  --> name of the bot
local own_botdescription = ""  --> bot description (only for own botname)
...
local FreshStuff = {
  ...
  Levels = {
	...
	ReLoad = 55,  --> reload
	Search = 20,  --> search
	Prune = 55,  --> prune (delete old)
	TopAdders = 20,  --> top release adders
	Help = 20,  --> guess what! :)
	Announce = 20,  --> adding with other nickname
	OptOut = 20, --> opt out
	OptIn = 20, --> opt in
  },

	MaxItemAge = 60,  --> for the "prunerel" command (delete old) (in DAYS)
	AutomatedClean = true,	--> Release clean (delete old) every day change (true/false)
	TopAddersCount = 20,  --> shows top-adders on command, this is the number how many it should show
	ShowOnEntry = 0,  --> Show latest stuff on entry 1=PM, 2=MAIN, 0=DISABLED
...
local ucmd_optout = lang.ucmd_optout or "Turn OFF release announcement"
local ucmd_optin = lang.ucmd_optin or "Turn ON release announcement"
...
-- handler signature is (user, cmd, parameters, txt)
local onbmsg = function(user, cmd, parameters, txt)
    local cmd_lower = string_lower(cmd)
    if commandtable[cmd_lower] then
        parsecmds(user, txt, "MAIN", cmd_lower, botname())
        return PROCESSED
    end
end
...
hub.setlistener( "onStart", { },
  function( p )
   ...
   if hubcmd then
       for _, cmd in pairs(FreshStuff.Commands) do
           hubcmd.add(cmd, onbmsg)
       end
   end
    ...
    ucmd.add( { ucmd_menu,"TV","720p-WEBRip",ucmd_add }, cmd_Add, { "TV_720p_WEBRip", "%[line:Name:]" }, { "CT1" }, Add )
    ucmd.add( { ucmd_menu,"TV","720p-WEBRip",ucmd_show }, cmd_Show, { "TV_720p_WEBRip"}, { "CT1" }, Show )
    ...
    ucmd.add( { ucmd_menu,"TV","1080p-WEBRip",ucmd_add }, cmd_Add, { "TV_1080p_WEBRip", "%[line:Name:]" }, { "CT1" }, Add )
    ucmd.add( { ucmd_menu,"TV","1080p-WEBRip",ucmd_show }, cmd_Show, { "TV_1080p_WEBRip"}, { "CT1" }, Show )
