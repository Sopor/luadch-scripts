...
local scriptname = "etc_mainecho.lua"
...
local minlevel = 60  -- lowest level for using the sleep command
...
local trigger_delay = 3  -- delay for trigger reply message in seconds

local trigger_cooldown = 10  -- delay for same trigger in minutes

local sleep_delay = 60  -- bot sleep time after sleep command in minutes
    ...
    function( user, adccmd, txt )
        ...
        if permission[ user_level ] then
            for k, v in pairs( echo_tbl ) do
                ...
                if s then
                    local now = os.time()
                    local cooldown = trigger_cooldown * 60
                
                    if ( not trigger_last_used[k] ) or
                       ( now - trigger_last_used[k] >= cooldown ) then
                
                        trigger_last_used[k] = now
