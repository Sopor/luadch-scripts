--[[
    ...
    usr_hubs.lua by blastbeat
        ...
        v0.13: by Sopor
            - shows user's IP address in OpChat
...
local report_msg = lang.report_msg or "[ USER HUBS ]--> User:  %s  |  with IP: %s  |  was banned for:  %s  |  reason: exceeded users hub limit. Hubs:  %s  (total:  %s hubs)  |  allowed:  %s  (max.  %s  hubs total)"
local report_msg_redirect = lang.report_msg_redirect or "[ USER HUBS ]--> User:  %s  |  with IP: %s  |  was redirected  |  reason: exceeded users hub limit. Hubs:  %s  (total:  %s hubs)  |  allowed:  %s  (max.  %s  hubs total)"
...
local check = function( user )
    ...
    elseif ( hn > user_max ) or ( hr > reg_max ) or ( ho > op_max ) or ( hm > hubs_max ) then
        ...
        if usr_hubs_redirect then
            ...
            --// report
            local msg_out = utf.format( report_msg, user_nick, user_ip, msg_bantime, hubs, hm, hubs_allowed, hubs_max )
            report.send( report_activate, report_hubbot, report_opchat, llevel, msg_out )
            return PROCESSED
        end
    end
