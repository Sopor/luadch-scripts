...
local allowed_percentage = 0 --// percentage between prefix and upload speed //--
local opchat_interval = 3600 --// time before the next OP message and when the user will be blocked //--
...
local matchUser = function(user)
    ...
    if tonumber(USpeed) > USpeedPrefix then
        ...
        if not warn_time[nick] then
            warn_time[nick] = os.time()
            if user and type(user.reply) == "function" then
                pcall(function()
                    user:reply("\n\n[ SPEED PREFIX ]--> WARNING: your UPLOAD SPEED (" .. USpeedString .. ") is higher than your PREFIX (" .. tostring(speed_prefix) .. "). Fix this within " .. tostring(math.ceil(opchat_interval / 60)) .. " minute(s) or you will be blocked.\n\n" ..
                    "[ SPEED PREFIX ]--> VARNING: din UPPLADDNINGSHASTIGHET (" .. USpeedString .. ") är högre än ditt PREFIX (" .. tostring(speed_prefix) .. "). Fixa detta inom " .. tostring(math.ceil(opchat_interval / 60)) .. " minut(er) eller så kommer du att bli blockerad.\n", getbot(), getbot())
                end)
            end
        end
        return nil
    ...
    if (diff * 100 / USpeedPrefix) > allowed_percentage then
        ...
        if not warn_time[nick] then
            ...
            if user and type(user.reply) == "function" then
                pcall(function()
                    user:reply("\n\n[ SPEED PREFIX ]--> WARNING: your PREFIX (" .. tostring(speed_prefix) .. ") is higher than your UPLOAD SPEED (" .. USpeedString .. "). Fix this within " .. tostring(math.ceil(opchat_interval / 60)) .. " minute(s) or you will be blocked.\n\n" ..
                    "[ SPEED PREFIX ]--> VARNING: ditt PREFIX (" .. tostring(speed_prefix) .. ") är högre än din UPPLADDNINGSHASTIGHET (" .. USpeedString .. "). Fixa detta inom " .. tostring(math.ceil(opchat_interval / 60)) .. " minut(er) eller du kommer att bli blockerad.\n", getbot(), getbot())
                end)
            end
        end
    else
        ...
        local firstnick = user:firstnick() or nick
        ...
        if ok and type(blocked_tbl) == "table" and blocked_tbl[firstnick] ~= nil then
            if block and type(block.del) == "function" then
                local fake_user = { nick = function() return scriptname end }
                local ok2, res2 = pcall(function() return block.del(firstnick, nil, fake_user) end)
                hub_debug(scriptname .. ": block.del pcall ok=" .. tostring(ok2) .. " res=" .. tostring(res2))
...
hub.setlistener("onTimer", {},
    function()
        ...
        for nick, msg in pairs(mismatch_state) do
            if not last_announce[nick] or (now - last_announce[nick]) >= opchat_interval then
            ...
            if not w or (now - w) < opchat_interval then
            else
                ...
                if target_user and type(target_user.firstnick) == "function" then
                    target_firstnick = target_user:firstnick()
                else
                    local stripped = string.match(nick, "^%b[]%s*(.+)$")
                    if stripped and stripped ~= "" then
                        target_firstnick = stripped
                    end
                end
                if block and type(block.add) == "function" then
                    local reason = "Upload speed mismatch - PREFIX vs UPLOAD SPEED  |  blocked by scriptname: " .. scriptname
                    local fake_user = { nick = function() return scriptname end }
                    local ok, res = pcall(function() return block.add(target_firstnick, nil, reason, fake_user) end)
                    hub_debug(scriptname .. ": block.add pcall ok=" .. tostring(ok) .. " res=" .. tostring(res))
                    local block_file = "scripts/data/etc_trafficmanager.tbl"
